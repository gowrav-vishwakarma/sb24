...
class CommunityProfileController extends CommunityBaseController
{
	...
	public function ajaxUploadNewPicture($userId)
	{
		...
		$content	.=	'<input class="cButton" type="file" id="file-upload" name="Filedata" />';
		...
		$actions = '<input type="button" value="'.JText::_('COM_COMMUNITY_BUTTON_UPLOAD_PICTURE').'" class="button" onclick="joms.jQuery(\'#uploadForm\').submit();" />';
			...
			else
			{
				...
				else
				{
					...
					if($currentWidth < COMMUNITY_AVATAR_MINIMUM_WIDTH || $currentHeight < COMMUNITY_AVATAR_MINIMUM_HEIGHT){
						$mainframe->enqueueMessage(JText::sprintf('COM_COMMUNITY_ERROR_MINIMUM_AVATAR_DIMENSION', COMMUNITY_AVATAR_MINIMUM_WIDTH, COMMUNITY_AVATAR_MINIMUM_HEIGHT), 'error');
