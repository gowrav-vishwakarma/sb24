...
class CommunityViewProfile extends CommunityView {
							...
							switch($field['type'])
							{
								...
								case 'checkbox':
									...
									foreach( $checkboxArray as $item ){
										if( !empty($item) )
											$profileField[$key][$pKey]['searchLink'][$item] = CRoute::_('index.php?'.urlencode('option=com_community&view=search&task=field&'.$field['fieldcode'].'='. urlencode( $item ) . '&type='.$field['type'] ));
									}
									break;
								case 'country':
									$profileField[$key][$pKey]['searchLink'] = CRoute::_('index.php?'.urlencode('option=com_community&view=search&task=field&'.$field['fieldcode'].'='. urlencode( $field['value'] ) ));
	...
	public function profile(& $data)
	{
		...
		$actId		= $jinput->get->get('actid' , '' , 'INT');
		...
		require_once (JPATH_COMPONENT.'/libraries/userpoints.php');
	...
	public function modProfileUserstatus(){
		...
		if( $my->id
			&&	((!$config->get('lockprofilewalls')) || ( $config->get('lockprofilewalls')
			&& CFriendsHelper::isConnected( $my->id , $user->id ) ) )
			||COwnerHelper::isCommunityAdmin()) {
