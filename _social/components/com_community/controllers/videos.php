...
class CommunityVideosController extends CommunityBaseController
{
	...
	public function link()
	{
		...
		$video->set('category_id', $jinput->post->get('category_id', '1', 'INT')) ; //JRequest::getVar('category_id', '1', 'POST'));
		$video->set('location', $jinput->post->get('location', '', 'STRING')); //JRequest::getVar('location', '', 'POST'));
		...
		if (empty($video->id) || !$video->tagAllow($my->id)) {
			...
			if ($tagId > 0) {
				$jsonString = '{'
					...
					. 'canRemove:true'
					. '}';
				$response->addScriptCall('joms.videos.addVideoTextTag', $jsonString);
