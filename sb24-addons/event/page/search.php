		...
		$form = $this->add('SearchForm',array('fields'=>$fields,'chain_fields'=>$chain_fields));
		 ...
		 $form->setFormClass('stacked atk-row');
            $o=$form->add('Order')
                ...
                // ->move($form->addSeparator('noborder atk-row'),'after','event_to_date')
                // ->move($form->addSeparator('noborder atk-row'),'after','tehsil_id')
                // ->move($form->addSeparator('noborder atk-row'),'after','event_type_id')
                ->now();

                $state_field=$form->getElement('state_id')->template->trySet("row_class",'span4');
                $event_date_field=$form->getElement('event_from_date')->template->trySet("row_class",'span4');
                $city_field=$form->getElement('city_id')->template->trySet("row_class",'span4');
                $tehsil_field=$form->getElement('tehsil_id')->template->trySet("row_class",'span4');
                $eventtype_field=$form->getElement('event_type_id')->template->trySet("row_class",'span4');
                $eventtype_field=$form->getElement('event_to_date')->template->trySet("row_class",'span4');
